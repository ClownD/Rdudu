...
//    
    ...
    anotherView.frame = CGRectMake(0, 244, self.view.frame.size.width, self.view.frame.size.height);
    [self.view addSubview:anotherView];
    UIWebView* webView = [[UIWebView alloc]initWithFrame:self.view.bounds];
    webView.scalesPageToFit = YES;
    //的限制网页在屏幕尺寸
    [self.view addSubview:webView];
    NSURL *url = [NSURL URLWithString:@"http://m.wufazhuce.com/article"];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    [webView loadRequest:request];
    [anotherView addSubview:webView];
    
    
//    self.tableView = [[UITableView alloc] init];
//    _tableView.frame = CGRectMake(0, 244, self.view.frame.size.width, self.view.frame.size.height);
//    self.tableView.delegate = self;
//    self.tableView.dataSource = self;
//    [self.view addSubview:_tableView];
    
//    [self.tableView registerClass:[BookTextViewCell class] forCellReuseIdentifier:@"cell"];
...
//    
////    BookTextViewCell *cell  =[tableView dequeueReusableCellWithIdentifier:@"cell"];
////    
////    
////    
